...
ENABLE_SAVE_CSV = getattr(config, 'ENABLE_SAVE_CSV', 1)
ENABLE_SAVE_JSON = getattr(config, 'ENABLE_SAVE_JSON', 1)
ENABLE_SAVE_SQLITE = getattr(config, 'ENABLE_SAVE_SQLITE', 0)
ONLY_ORIGINAL = getattr(config, "ONLY_ORIGINAL", 1)
ENABLE_SAVE_IMAGES = getattr(config, "ENABLE_SAVE_IMAGES", 1)
ENABLE_SAVE_VIDEOS = getattr(config, "ENABLE_SAVE_VIDEOS", 1)
ENABLE_SAVE_LIVEPHOTOS = getattr(config, "ENABLE_SAVE_LIVEPHOTOS", 1)
ENABLE_SAVE_COMMENT_MEDIA = getattr(config, "ENABLE_SAVE_COMMENT_MEDIA", 1)
...
DOWNLOAD_NUM_THREADS = getattr(config, "DOWNLOAD_NUM_THREADS", 10)
DOWNLOAD_NUM_CONCURRENT_MEDIA = getattr(config, "DOWNLOAD_NUM_CONCURRENT_MEDIA", 20)
ENABLE_SCRAPE_COMMENTS = getattr(config, "ENABLE_SCRAPE_COMMENTS", 1)
MAX_COMMENTS_PER_POST = getattr(config, "MAX_COMMENTS_PER_POST", 20)
MAX_REPLIES_PER_COMMENT = getattr(config, "MAX_REPLIES_PER_COMMENT", 5)
COMMENT_FLOW = getattr(config, "COMMENT_FLOW", 0)
REPLY_PAGE_DELAY = getattr(config, "REPLY_PAGE_DELAY", 1)
SAVE_DATA_BY_PERIOD = getattr(config, "SAVE_DATA_BY_PERIOD", 0)
ENABLE_SAVE_MARKDOWN = getattr(config, 'ENABLE_SAVE_MARKDOWN', 1)
CRAWL_COMMENTS = getattr(config, 'CRAWL_COMMENTS', 1)
COMMENTS_LIMIT = getattr(config, 'COMMENTS_LIMIT', 10)
...
class WeiboAuth:
    ...
    def validate_or_login(self, page, context, headless=False):
        """
        统一全局登录有效性校验（拦截登录过期）。
        如果 headless=True，将在终端生成 ASCII 二维码供手机扫码。
        否则提示用户在弹出的浏览器中手动扫码。
        """
        original_print("正在校验微博登录状态...")
        try:
            page.goto("https://s.weibo.com/weibo?q=check", wait_until="domcontentloaded", timeout=15000)
            # 使用 s.weibo.com 会立即进行服务器端跳转，比客户端 JS 跳转更可靠

            
            if "passport.weibo.com" in page.url or "login" in page.url or "retcode=6102" in page.url:
                print_rich("\n⚠️ [bold red]检测到微博登录状态已失效！[/bold red]")
                print_rich("👉 [bold yellow]请运行 `python login.py` 进行扫码登录，登录成功后再运行本程序。[/bold yellow]")
                sys.exit(1)
...
class WeiboStorage:
    ...
    def download_file(self, url, save_path, page=None, show_progress=True):
        """
    ...
    def extract_high_quality_video(self, detail_url, headless_browser):
        if not detail_url:
            return ""
        ...
        try:
            ...
            if valid_urls:
                ...
                best_score = self.get_url_score(best_url)
    ...
    def extract_comment_media_url(self, item):
        """
    ...
    def parse_markdown_posts(self, file_path):
        """
    ...
    def save_to_csv(self, data, user_name, target_dir=None, suffix=""):
        """
    ...
    def save_to_sqlite(self, data, user_name, target_dir=None, suffix=""):
        """
    ...
    def save_to_json(self, data, user_name, target_dir=None, suffix=""):
        """
    ...
    def save_data(self, data, user_name, page=None):
        # 按日期分组
        posts_by_date = {}
        for post in data:
        ...
        for date_str, new_posts in posts_by_date.items():
                    ...
                    if tasks:
                        ...
                        with Progress(
                            ...
                            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                                def worker(task):
                                    success = self.download_file(task["url"], task["save_path"], page, show_progress=False)
                                    return task, success
            ...
            if ENABLE_SAVE_MARKDOWN:
                # 2. 读取并解析旧微博归档
                old_md_posts = self.parse_markdown_posts(file_path)
        ...
        if SAVE_DATA_BY_PERIOD in ("global", "both"):
            if ENABLE_SAVE_CSV:
                try:
                    self.save_to_csv(data, user_name)
                except Exception as csv_err:
                    print(f"保存全局 CSV 失败: {csv_err}")
                
            if ENABLE_SAVE_SQLITE:
                try:
                    self.save_to_sqlite(data, user_name)
                except Exception as sqlite_err:
                    print(f"保存全局 SQLite 失败: {sqlite_err}")
    
            if ENABLE_SAVE_JSON:
                try:
                    self.save_to_json(data, user_name)
        ...
        if SAVE_DATA_BY_PERIOD in ("monthly", "both"):
            ...
            for month_str, month_posts in posts_by_month.items():
                ...
                if ENABLE_SAVE_CSV:
                    try:
                        self.save_to_csv(month_posts, user_name, target_dir=month_dir, suffix=month_str)
                    except Exception as csv_err:
                        print(f"保存月度 CSV ({month_str}) 失败: {csv_err}")
                    
                if ENABLE_SAVE_SQLITE:
                    try:
                        self.save_to_sqlite(month_posts, user_name, target_dir=month_dir, suffix=month_str)
                    except Exception as sqlite_err:
                        print(f"保存月度 SQLite ({month_str}) 失败: {sqlite_err}")
                    
                if ENABLE_SAVE_JSON:
                    try:
                        self.save_to_json(month_posts, user_name, target_dir=month_dir, suffix=month_str)
                    except Exception as json_err:
                        print(f"保存月度 JSON ({month_str}) 失败: {json_err}")
            


    def delete_local_post_data(self, post_id, target_uid=None):
        """
        ...
        for ud in user_dirs:
            ...
            for file_path in target_files:
                ...
                # 处理 Markdown 文件
                    ...
                    try:
                        posts = self.parse_markdown_posts(file_path)
    ...
    def delete_local_data_by_date(self, target_date, target_uid=None):
        """
...
class WeiboCrawler:
    ...
    def scrape_replies(self, page, post_id, comment_id, post_author_uid=""):
        """
        ...
        while len(replies) < max_replies_limit:
            ...
            try:
                ...
                for item in data:
                    ...
                    media_url = self.storage.extract_comment_media_url(item)
    ...
    def scrape_comments(self, page, post_id, target_user_id=""):
        """
        ...
        for data in collected_json_data:
            for item in data:
                ...
                if total_replies_cnt > len(raw_replies) and len(raw_replies) < MAX_REPLIES_PER_COMMENT:
                    replies = self.scrape_replies(page, post_id, comment_id, target_user_id)
                else:
                    for r_item in raw_replies:
                        ...
                        r_media_url = self.storage.extract_comment_media_url(r_item)
                        replies.append({
                            "id": r_id,
                            "post_id": post_id,
                            "parent_id": comment_id,
    ...
    def fetch_user_name(self, page, user_id):
        """
    ...
    def scrape_and_save_user_profile(self, page, user_id, user_name):
        """
        ...
        if str(user_name) != str(user_id) and os.path.exists(file_path):
            print(f"ℹ️ 用户 {user_name} ({user_id}) 的个人资料已存在，跳过重复提取下载。")
            return user_name
        ...
        try:
            ...
            if avatar_clean:
                ...
                if not os.path.exists(avatar_local_path):
                    try:
                        self.storage.download_file(avatar_clean, avatar_local_path, show_progress=False)
            ...
            if avatar_hd_clean:
                ...
                if not os.path.exists(avatar_hd_local_path):
                    try:
                        self.storage.download_file(avatar_hd_clean, avatar_hd_local_path, show_progress=False)
            ...
            if cover_phone:
                ...
                if not os.path.exists(cover_phone_local_path):
                    try:
                        self.storage.download_file(cover_phone, cover_phone_local_path, show_progress=False)
            ...
            if cover_web and cover_web != cover_phone:
                ...
                if not os.path.exists(cover_web_local_path):
                    try:
                        self.storage.download_file(cover_web, cover_web_local_path, show_progress=False)
    ...
    def update_userid_file(self, file_path, user_id, username, timestamp_str):
        """
    ...
    def load_existing_post_ids(self, user_name):
        """
                ...
                for root, dirs, files in os.walk(user_dir):
                    for file in files:
                            if file.endswith(".md"):
                                ...
                                try:
                                    posts = self.storage.parse_markdown_posts(md_path)
                                    for p in posts:
                                        if p.get("id"):
    ...
    def scrape_weibo_search(self, scrape_target=None, target_uid=None, headless=False):
        ...
        with sync_playwright() as p:
            ...
            # 统一全局登录有效性校验（拦截登录过期）
            self.auth.validate_or_login(page, context, headless=headless)
            ...
            with global_dashboard:
                for user_idx, user_info in enumerate(user_ids, 1):
                    ...
                    # 自动获取用户昵称
                    user_name = self.fetch_user_name(page, user_id)
                ...
                real_name = self.scrape_and_save_user_profile(page, user_id, user_name)
                ...
                scraped_ids = self.load_existing_post_ids(user_name)
                ...
                for start_str, end_str in user_date_ranges:
                    ...
                    if scrape_target and not re.match(r'^\d{4}-\d{2}-\d{2}$', scrape_target):
                        ...
                        if post_id_input.isdigit():
                            ...
                            for i in range(len(bid) - 4, -4, -4):
                                ...
                                if length == 0 and offset == 0: length = 4
                                chunk = bid[offset:offset+length]
                                num = self.storage.base62_decode(chunk)
                                if offset > 0: mid = str(num).zfill(7) + mid
                                else: mid = str(num) + mid
                            target_mid = mid
                        ...
                        for card in cards:
                            try:
                                        ...
                                        if video_el.is_visible():
                                            ...
                                            if data_str:
                                                ...
                                                if address_match:
                                                    ...
                                                    if detail_url.startswith("//"):
                                                        detail_url = "https:" + detail_url
                                                    video_url = self.storage.extract_high_quality_video(detail_url, headless_browser)
                                ...
                                if ENABLE_SCRAPE_COMMENTS and post_id:
                                    try:
                                        comments = self.scrape_comments(page, post_id, user_id)
                                ...
                                try:
                                    ...
                                    try:
                                        self.storage.save_data([post_data], user_name, page)
                                    finally:
                    ...
                    if start_str != "SINGLE_POST":
                        global_dashboard.stop()
                        print_rich(f"  [green]✅ {start_str} 共保存了 {day_saved} 条新微博[/green]")
                        global_dashboard.start()
                    
                    # 每次完成一天的抓取后，写入对应日期的增量游标以支持崩溃后断点续爬 (单篇/单日指定时不在此更新)
                    if start_str != "SINGLE_POST" and TARGET_USER_IDS.endswith(".txt") and not scrape_target:
                        today_str = datetime.now().strftime("%Y-%m-%d")
                        
                        if end_str == today_str:
                            crawl_now = datetime.now().replace(microsecond=0)
                            cursor_time_str = crawl_now.strftime("%Y-%m-%dT%H:%M:%S")
                        else:
                            cursor_time_str = f"{end_str}T23:59:59"
                        
                        self.update_userid_file(TARGET_USER_IDS, user_id, user_name, cursor_time_str)
                ...
                if TARGET_USER_IDS.endswith(".txt") and not scrape_target:
                    today_str = datetime.now().strftime("%Y-%m-%d")
                    if local_end_date and local_end_date != today_str:
                        cursor_time_str = f"{local_end_date}T23:59:59"
                    else:
                        crawl_end_time = datetime.now().replace(microsecond=0)
                        cursor_time_str = crawl_end_time.strftime("%Y-%m-%dT%H:%M:%S")
                    self.update_userid_file(TARGET_USER_IDS, user_id, user_name, cursor_time_str)
...
def main():
    ...
    parser.add_argument("-d", "--delete", help="删除指定日期或指定短 ID 微博的所有本地数据 (格式: YYYY-MM-DD 或 短ID)")
    parser.add_argument("-s", "--scrape", help="仅爬取指定日期的新微博并保存 (格式: YYYY-MM-DD) 或 指定短 ID/网页链接 (爬取单条)")
    parser.add_argument("-u", "--uid", help="指定目标用户的 UID (当使用 -d 且 target_user_ids.txt 有多个配置时必须指定)")
    parser.add_argument("--headless", action="store_true", help="开启纯无头模式 (服务器/NAS使用，支持终端扫码)")
    
    args = parser.parse_args()

    auth_manager = WeiboAuth()
    storage_manager = WeiboStorage()
    crawler = WeiboCrawler(auth_manager, storage_manager)

    # 优先处理删除操作
    if args.delete:
        print_rich("\n[bold red]=== 警告: 准备执行删除操作 ===[/bold red]")
        if re.match(r'^\d{4}-\d{2}-\d{2}$', args.delete):
            storage_manager.delete_local_data_by_date(target_date=args.delete, target_uid=args.uid)
        else:
            # check if it's a url or id
            delete_target = args.delete
            if "weibo.com" in delete_target:
                parts = delete_target.split("/")
                delete_target = parts[-1]
            storage_manager.delete_local_post_data(post_id=delete_target, target_uid=args.uid)
        return

    # 增量或指定抓取操作
    if args.scrape:
        crawler.scrape_weibo_search(scrape_target=args.scrape, target_uid=args.uid, headless=args.headless)
    else:
        crawler.scrape_weibo_search(headless=args.headless)
