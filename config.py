...
INCREMENTAL_LOOKBACK_DAYS = 0   # 增量爬取回溯天数。在增量抓取时，将起点向前推指定天数，以防因发布/索引延迟漏掉微博。已存在的微博会自动按 ID 去重。
...
ENABLE_SAVE_CSV = 0             # 是否保存微博文本数据为 CSV 文件 (1 = 启用，0 = 禁用)
...
MAX_REPLIES_PER_COMMENT = 200     # 每条主评论最多爬取的子回复数 (楼中楼)
